...
class ApplesToApples(BasePlugin):
    ...
    def cmd_rules(self, args, channel, user):
        ...
        self.reply(channel, user, "After all players have put down a card, the Judge gets to pick which one they like best, with !pick [number]. There really isn't any set criteria to determine the 'best' card. The judge just picks the one they like the most! It could be the one that fits the green card best, or the one they find the funniest, or the most random. It's up to the judge! Players must learn what the judges like, and use that to their advantage.")
